...
const buildPointer = await (await context.request.get(new URL('browser/current.json', siteBase).href)).json();
const buildManifest = await (await context.request.get(new URL(`${buildPointer.build}build-info.json`, siteBase).href)).json();
if (buildManifest.entrypoint === 'mockui') {
  console.log(JSON.stringify({ result: 'pass', camera: 'not exposed by Playground MockUI' }));
  await browser.close();
  process.exit(0);
}
